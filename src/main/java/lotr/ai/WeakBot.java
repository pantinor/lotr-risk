...
public class WeakBot extends BaseBot {
    ...
    public WeakBot(Game game, Army army, GameScreen gameScreen) {
    ...
    public TerritoryCard findClaimedTerritory(boolean mayInvadeFrom) {
        ...
        return sorted.get(sorted.size() - 1).territory;
    }
    ...
    public void reinforce(TerritoryCard country) {
        ...
        if (territoryReinforcements > 0) {
            ...
            //evenly spread out battalions
            Collections.sort(sorted);
            for (int i = 0; i < territoryReinforcements; i++) {
                army.addBattalion(sorted.get(i >= sorted.size() ? 0 : i).territory);
            }
        }
